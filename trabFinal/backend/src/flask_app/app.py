...
class AppFunctions():
    ...
    def criarUsuario(self, json_usr):
        ...
        return self.usuarioDAO.addUsuario(usuario)
    ...
    def criarAeroporto(self, json_aeroporto):
        ...
        return self.aeroportoDAO.insertAeroporto(aeroporto)
    ...
    def deletePassagem(self, idPassagem):
        ...
        return self.passagemDAO.deletePassagemByIDs([idPassagem])[0]
    ...
    def updateVooById(self, json_voo):
        # percorre array
        # adiciona as colunas que faltam
        # manda pro delete
        voo = self.vooDAO.getVooById(id)

        if (json_voo.get('partida') is None):
            json_voo["partida"] = voo.getIdAeroportoSaida()
        
        if (json_voo.get('destino') is None):
            json_voo["destino"] = voo.getIdAeroportoChegada()
        
        if (json_voo.get('diaPartida') is None):
            json_voo["diaPartida"] = voo.getDiaPartida()
        
        if (json_voo.get('horarioPartida') is None):
            json_voo["horarioPartida"] = voo.getHorarioPartida()
        
        if (json_voo.get('diaChegada') is None):
            json_voo["diaChegada"] = voo.getDiaChegada()
        
        if (json_voo.get('horarioChegada') is None):
            json_voo["horarioChegada"] = voo.getHorarioChegada()
        
        if (json_voo.get('valor') is None):
            json_voo["valor"] = voo.getPreco()
        
        if (json_voo.get('passagens') is None):
            json_voo["passagens"] = voo.getQtLugaresDisponiveis()
    ...
    def listarVoosDisponiveis(self, nPessoas=1):
        voos = self.vooDAO.getTbVoo()
        voos_json = []
        for voo in voos:
