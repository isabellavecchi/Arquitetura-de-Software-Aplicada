...
# from models import User
from database.connectDB import ConectaBD
...
def listarPassagens():
    ...
    ## Aeroporto
@app.route('/aeroportos/deletar/id', methods=['DELETE'])
def deletePassagem():
    id = request.json
    if(funcApp.deletePassagem(id)):
        logging.info('delete passagem de id: ', id)
    ...
    ## Voos
@app.route('/voos/deletar/id', methods=['DELETE'])
def deleteVoo():
    id = request.json
    if(funcApp.deleteVoo(id)):
        logging.info('delete deleteVoo de id: ', id)

        return jsonify({"msg":"voo e suas passagens correspondentes deletades com sucesso!"})

    return jsonify({"msg":"xabu ao deletar voo..."})
...
def deleteAeroporto():
    id = request.json
    if(funcApp.deleteVoo(id)):
        logging.info('delete Aeroporto de id: ', id)

        return jsonify({"msg":"aeroporto deletado com sucesso!"})

    return jsonify({"msg":"xabu ao deletar aeroporto..."})
